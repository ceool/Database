-- 테이블을 생성한 후 제약 조건을 추가하거나 제거하고 싶다면 alter 구문을 이용한다

-- alter table 테이블명
-- add 제약조건

-- alter table 테이블명
-- drop 제약조건


-- 제약조건 활성/비활성화: diable을 사용하면 제약 조건이 비활성화되고, enable을 사용하면 제약조건이 활성화 된다.
-- 주의점: 제약조건을 활성화할 때, 제약조건에 위배되면 활성화 되지않음.

-- alter table 테이블명
-- enble 제약조건

-- alter table 테이블명
-- disable 제약조건


drop table test_table20;
drop table test_table30;
drop table test_table40;


--제약 조건 추가
create table test_table20(
    data1 number,
    data2 number,
    data3 number,
    data4 number,
    data5 number,
    data6 number
);

-- null 제약 조건 추가
alter table test_table20
modify data1 not null;

-- ORA-01400: NULL을 ("SCOTT"."TEST_TABLE20"."DATA1") 안에 삽입할 수 없습니다
insert into test_table20 (data1)
values(null);




-- primary key 제약 조건 추가
alter table test_table20
add constraint TEST_TABLE20_DATA2_PK primary key(data2);

insert into test_table20 (data1, data2)
values(1, 10);

-- ORA-00001: 무결성 제약 조건(SCOTT.TEST_TABLE20_DATA2_PK)에 위배됩니다
insert into test_table20 (data1, data2)
values(2, 10);

-- ORA-01400: NULL을 ("SCOTT"."TEST_TABLE20"."DATA2") 안에 삽입할 수 없습니다
insert into test_table20 (data1, data2)
values(3, null);




-- 외래키 제약 조건 추가
alter table test_table20
add CONSTRAINT TEST_TABLE20_FK FOREIGN key(data3)
                         REFERENCES emp(empno);
                         
select empno from emp;

insert into test_table20 (data1, data2, data3)
values(10, 100, 7369);

-- ORA-02291: 무결성 제약조건(SCOTT.TEST_TABLE_20_FK)이 위배되었습니다- 부모 키가 없습니다
insert into test_table20 (data1, data2, data3)
values(11, 101, 100);




-- unique 제약 조건 추가
alter table test_table20
add CONSTRAINT TEST_TABLE20_DATA4 UNIQUE(data4);

insert into test_table20 (data1, data2, data4)
values(12, 102, 100);

-- ORA-00001: 무결성 제약 조건(SCOTT.TEST_TABLE20_DATA4)에 위배됩니다
insert into test_table20 (data1, data2, data4)
values(13, 103, 100);




-- check 제약 조건
alter table test_table20
add constraint TEST_TABLE20_DATA5_CK CHECK(data5 between 1 and 10);

insert into test_table20 (data1, data2, data5)
values (14, 104, 5);

-- ORA-02290: 체크 제약조건(SCOTT.TEST_TABLE20_DATA5_CK)이 위배되었습니다
insert into test_table20 (data1, data2, data5)
values (15, 105, 11);





alter table test_table20
add constraint TEST_TABLE20_DATA6_CK check(data6 in (10, 20, 30));

insert into test_table20 (data1, data2, data6)
values(16, 106, 20);

-- ORA-02290: 체크 제약조건(SCOTT.TEST_TABLE20_DATA6_CK)이 위배되었습니다
insert into test_table20 (data1, data2, data6)
values(17, 107, 21);


select * from test_table20;





-- 제약조건 삭제
create table test_table30(
    data1 number not null,
    data2 number constraint TEST_TABLE30_DATA2_PK primary key,
    data3 number constraint TEST_TABLE30_DATA3_FK
                                       references emp(empno),
    data4 number constraint TEST_TABLE30_DATA4_UK unique,
    data5 number constraint TEST_TABLE30_DATA5_CK check (data5 between 1 and 10),
    data6 number constraint TEST_TABLE30_DATA6_CK check (data6 in (10,20,30))
);


-- not null 제약 조건 제거
alter table test_table30
modify data1 null;

insert into test_table30(data1, data2)
values(null, 100);


-- primary key 제약 조건 제거
alter table test_table30
drop constraint TEST_TABLE30_DATA2_PK;

insert into test_table30(data2)
values(null);


-- 외래키 제약 조건 제거
alter table test_table30
drop constraint TEST_TABLE30_DATA3_FK;

insert into test_table30(data3)
values(1000);


-- unique 제약 조건 제거
alter table test_table30
drop constraint TEST_TABLE30_DATA4_UK;

insert into test_table30(data4)
values(100);

insert into test_table30(data4)
values(100);


-- 체크 제약 조건 제거
alter table test_table30
drop constraint TEST_TABLE30_DATA5_CK;

alter table test_table30
drop constraint TEST_TABLE30_DATA6_CK;

insert into test_table30(data5, data6)
values (20, 100);


select * from test_table30;





create table test_table40 (
    data1 number constraint TEST_TABLE40_DATA1_PK primary key
);

insert into test_table40 (data1)
values(100);

-- ORA-00001: 무결성 제약 조건(SCOTT.TEST_TABLE40_DATA1_PK)에 위배됩니다
insert into test_table40 (data1)
values(100);


-- 제약조건 비활성화
alter table test_table40
disable constraint TEST_TABLE40_DATA1_PK;

insert into test_table40 (data1)
values(100);

select * from test_table40;


-- 제약조건 활성화(실패)
-- ORA-02437: (SCOTT.TEST_TABLE40_DATA1_PK)을 검증할 수 없습니다 - 잘못된 기본 키입니다
alter table test_table40
enable constraint TEST_TABLE40_DATA1_PK;

delete from test_table40;

insert into test_table40 (data1)
values(100);

insert into test_table40 (data1)
values(200);

select * from test_table40;


-- 제약조건 활성화(성공)
alter table test_table40
enable constraint TEST_TABLE40_DATA1_PK;

-- ORA-00001: 무결성 제약 조건(SCOTT.TEST_TABLE40_DATA1_PK)에 위배됩니다
insert into test_table40 (data1)
values(200);

