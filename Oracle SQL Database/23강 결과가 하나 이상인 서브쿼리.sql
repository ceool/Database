...
-- KING은 직속상관으로 가지고 있는 사원들이 근무하고 있는 근무 부서명, 지역을 가지고 온다.
select dname, loc
from dept
where deptno in (select deptno
                from emp
...
-- 각 부서별  최저급여 액수보다 많이 받는 사원들의 사원번호, 이름, 급여를 가져온다
select empno, ename, sal
from emp
...
-- DALLAS에 근무하고 있는 사원들 중 가장 나중에 입사한 사원의 입사 날짜보다 더 먼저 입사한 사원들의
-- 사원번호, 이름, 입사일을 가져온다
select empno, ename, sal
from emp
