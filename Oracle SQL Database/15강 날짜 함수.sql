...
select ename, job, hiredate, hiredate - 100
from emp
where job = 'SALESMAN';
...
-- 반올림
select sysdate, round(sysdate, 'CC') as "세기", round(sysdate, 'YYYY') as 월기준,
    round(sysdate, 'DD') as "시기준", round(sysdate, 'HH') as "분기준",
...
-- 버림
select sysdate, trunc(sysdate, 'CC') as "세기"
from dual;


-- 1981년에 입사한 사람들의 사원번호, 사원이름, 급여, 입사일을 가져온다
...
select trunc(months_between(sysdate, hiredate)) as "근무한 개월 수"
from emp;
...
-- 1900-10-10
select hiredate, to_char(hiredate, 'YYYY-MM-DD-HH-MI-SS AM')
from emp;